...
且题目讲明了只有一个重复的数存在，算法是基于这样的性质设f(x)是数组中小于等于x的数的数目，那么g(x)=x-f(x)只有小于0，大于0，等于0三种取值。如果最终答案是ans，当x < ans时，g(x) < 0，当x >= ans时，g(x) >= 0，这样看来也满足x增大，g(x)不减的性质，故可以通过二分x来搜索答案。
