...
`S1`中除了循环的部分，还会有起始和末尾两部分不属于循环节中的，这两个部分需要单独计算。若当前循环节是`i`到`j`，那么还需要计算`[0,i-1]`以及`[j+1,n1]`中`s2`出现的次数。如：`S1 = ["bacd",6] = "bacdbacdbacdbacdbacdbacd"`, `S2 = ["cda",2] = "cdacda"`，这样开头不属于循环节的部分就是`"ba"`，结尾不属于循环节部分就是`"cdbacd"`。
